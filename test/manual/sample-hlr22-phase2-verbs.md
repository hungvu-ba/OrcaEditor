...
### TC2.7d — In an `<ol>`: converting one item to a task list splits the list

<!-- WHEN: caret anywhere in "Bravo" ONLY (do not select all three), click
     the toolbar's Task List button.
     THEN: Bravo becomes its own single-item task list ("* [ ] Bravo") in its
     own `<ul class="contains-task-list">`. Alpha stays "1." and Charlie its
     own `<ol>` around it — both remain NUMBERED and do NOT shift left (the
     `<ol>`s must NOT get `contains-task-list`). The raw must never be
     "2. [ ] Bravo" (a number marker next to a checkbox). Mirror of TC2.3c
     (bug ExcelCmd #3). -->
